...
from legal_mcp.policy import AccessContext, user_group_ids, visible_project_ids
...
def _allowed_fields_by_domain(
    ...
    if not _has_field_grants(conn, access_context):
        return {
            domain: sorted(fields)
            for domain, fields in FIELDS_BY_DOMAIN.items()
        }

    group_ids = user_group_ids(conn, access_context)
